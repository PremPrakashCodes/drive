  ...
  return (
    <section className="mb-7.25" onClick={clearOnBackground}>
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <button
          className="flex items-center gap-1.75 text-[10px] text-muted-foreground md:text-[11px]"
      ...
      <div
        className="grid grid-cols-4 gap-3.75 max-[1200px]:gap-3 max-[1000px]:grid-cols-2"
        onClick={clearOnBackground}
      >
        {folders.map((f) => (
          <ContextMenu key={f.id}>
            <ContextMenuTrigger
              className="relative min-w-0 rounded-[9px] border bg-card px-3.75 pt-3.5 pb-3.25 transition-[border-color,translate] duration-150 ease-[ease] select-none hover:-translate-y-px hover:border-folder-green focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring aria-selected:translate-none aria-selected:border-primary/28! aria-selected:bg-[color-mix(in_srgb,var(--primary)_12%,var(--card))] max-md:p-3.25 min-[1600px]:p-4.5"
              tabIndex={0}
            ...
            >
              <div className="mb-3.75 flex items-center justify-between max-[1000px]:mb-2.25 max-md:gap-1.5">
                <button
                ...
                <FileMenu
                  ...
                  classes={{
                    mobile:
                      "h-5.75! w-5! text-muted-foreground hover:text-muted-foreground aria-expanded:text-muted-foreground max-md:min-h-9",
                  }}
                />
              </div>
              <button
              ...
              <div className="mt-1.75 flex justify-between text-[9px] text-muted-foreground md:text-[11px]">
