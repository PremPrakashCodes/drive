...
export function getActiveProvider(preferences: Preferences, workspace: string) {
  const id =
    ...
    storageProviders.find((p) => preferences[`${workspace}:provider:${p.id}`])
      ?.id;
...
export function disconnectProvider(
  ...
  return Object.fromEntries(
    Object.entries(preferences).filter(([key]) => {
      ...
      return !(
        keys.includes(name) || /^(provider|bucket|region):/.test(name)
      );
    }),
  );
}
...
export function connectProvider(
  ...
  if (active && active.provider.id !== connection.provider)
    throw new Error(
      `Disconnect ${active.provider.name} before connecting another provider.`,
    );
  return {
